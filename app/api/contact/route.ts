...
export async function POST(req: NextRequest) {
  ...
  const { nome, email, telefono, tipoProgetto, messaggio, honeypot } = body

  if (honeypot) {
    return NextResponse.json({ ok: true })
  }

  if (!nome?.trim() || !email?.trim() || !messaggio?.trim() || !tipoProgetto?.trim()) {
  ...
  try {
    await sendContactEmail({ nome, email, telefono, tipoProgetto, messaggio })
    return NextResponse.json({ ok: true })
  } catch (err) {
